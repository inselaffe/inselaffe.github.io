//	start:	COUNTRIES
//		extend for more markets as required
var globalCountryLookup = {
  gb: {
    domainName: "uk",
    timeParting: {hemisphere: "n", timeZone: "0", dst: "eur"},
  }, 
  ie: {
    domainName: "ie",
    timeParting: {hemisphere: "n", timeZone: "0", dst: "eur"},
  ...
  us: {
    domainName: "us",
    timeParting: {hemisphere: "n", timeZone: "-6", dst: "us"},
  },
  ca: {
    domainName: "ca",
    timeParting: {hemisphere: "n", timeZone: "-4", dst: "us"},
  }
}
//	end:	COUNTRIES

//	start:	SITE-SPECIFIC CONFIGURATION VARIABLES
var tp_env = "stag"	//	choose from production ("prod") or other ("stag") environments (e.g., development, testing, staging)
var tp_mkt = "gb"	//	modify to reflect market, based on 'globalCountryLookup'
var tp_countryInfo = globalCountryLookup[tp_mkt];
//	end:	SITE-SPECIFIC CONFIGURATION VARIABLES

//	start:	DEFAULT CONFIGURATION VARIABLES
var dyson_nm = "dyson"
var dyson_domains = Object.keys(globalCountryLookup)
var dyson_mkts = []
for (d in dyson_domains) {
	dyson_mkts.push(globalCountryLookup[dyson_domains[d]].domainName)
}
var dyson_envs = ["stag", "prod"]
var s_account = "dysonglobal"
//	end:	DEFAULT CONFIGURATION VARIABLES

//	CHECK CORRECT CONFIGURATION AND SET s_account
if(
	dyson_nm && tp_env && dyson_envs && (~dyson_envs.indexOf(tp_env)) && dyson_mkts && tp_mkt && (~dyson_mkts.indexOf(tp_mkt))
	)	{
	s_account = dyson_nm + tp_mkt + tp_env;
}

var s=s_gi(s_account)

/******** VISITOR ID SERVICE CONFIG - REQUIRES VisitorAPI.js ********/
s.visitor=Visitor.getInstance("2E7EC23B54BE6D810A4C98BC@AdobeOrg")

/* WARNING: Changing any of the below variables will cause drastic
changes to how your visitor data is collected.  Changes should only be
made when instructed to do so by your account manager.*/
s.trackingServer="dyson.d3.sc.omtrdc.net"
s.trackingServerSecure="dyson.d3.sc.omtrdc.net"
...
s.linkInternalFilters="javascript:,foresee.com"
...
(~d_url.indexOf(".co.") || ~d_url.indexOf(".com.")) ? s.cookieDomainPeriods = "3" : s.cookieDomainPeriods = "2";

s.usePlugins=true

//	get External Campaign params
var ext_campaign = [s.Util.getQueryParam('utm_campaign'), 
...
//	run a check that certain terms exist, then set if exist
if(ext_campaign[0] && ext_campaign[1] && ext_campaign[2]){
  s.campaign = ext_campaign.join(":").toLowerCase;  
}

function s_doPlugins(s) {
	//	get Marketing Cloud ID
	s.prop7 = Visitor.getInstance("2E7EC23B54BE6D810A4C98BC@AdobeOrg").getMarketingCloudVisitorID();
	
	//	get previous page name value
	try{
	  if (s.pageName != undefined && s.pageName != "" && typeof s.getPreviousValue!="undefined"){ 
		s.prop49=s.getPreviousValue(s.pageName,'gpv_pn'); 
		s.eVar49="D=c49";
	  }
	}catch(err){}
	
	//	variables to be included on all link-tracking calls
	var alwaysLinkTrack = ["campaign","eVar1","eVar50","eVar52","prop7","prop52","contextData.link_type"];	

	//	if a link-tracking call is detected ...
  if (s.linkType)	
  {
    s.contextData['link_type'] = s.linkType;	//	capture LT type
    if (typeof alwaysLinkTrack != undefined && alwaysLinkTrack.length > 0)	{
      for(var i = 0; i < alwaysLinkTrack.length; i++)	{
        //	configure LT call to include variables listed in 'alwaysLinkTrack'
		s.linkTrackVars = (s.linkTrackVars == "None" ? alwaysLinkTrack[i] : s.apl(s.linkTrackVars, alwaysLinkTrack[i], ",", 2));
      }
    }
  }
  else if(!s.linkType)	{
	  //	clear down LT config if not LT call
    s.contextData['link_type'] = "";
    s.linkTrackVars = "None";
  }
  ...
  var yyyy = today.getFullYear();
	//format the date and month fields appropriately
    if(dd<10) {
        dd='0'+dd
    } 
  ...
  today = yyyy+'-'+mm+'-'+dd
	//	get Time Parting
  var tp = s.getTimeParting(tp_countryInfo.timeParting.hemisphere, tp_countryInfo.timeParting.timeZone);
  s.eVar50 = tp.split("|")[1].toLowerCase()+ "|" +tp.split("|")[0].toLowerCase().split(" ")[0]+tp.split("|")[0].toLowerCase().split(" ")[1]+"|"+ today;
	//	get Market ID
  s.eVar52 = tp_countryInfo.domainName;
  s.prop52 = "D=v52";

  s.eVar4="D=g"	//	URL
  s.eVar5="D=r"	//	Referrer
  s.prop5="D=g"	//	URL
  s.prop7=visitor.getMarketingCloudVisitorID();	//	MCVID
  s.prop52="D=v52"	//	Time Parting
...
/*
 * Plugin: getTimeParting 3.4 - Set timeparting values based on time zone
 */
...
s._tpDstVersion = globalCountryLookup[tp_mkt].timeParting.dst;
if	(s._tpDstVersion && s._tpDstVersion != "undefined" && s._tpDstVersion != "none")	{
  s._tpDST = s['_tpDST_' + s._tpDstVersion] || false;
}
